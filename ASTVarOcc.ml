					...
					if (!evaluate) then
							begin
								...
								if (!level > !maxlevel) then
									maxlevel := !level;
								if ((string_of_loc loc) = !varloc) then
									varlevel := !level;
								evaluate := false;
								evaluated := true
							end;
					let string = "<var loc='"^(string_of_loc loc)^(*"' level='"^(string_of_int !level)^*)"'>"^(escape_string name)^"</var>"  in
		...
		begin
			let name = (escape_string name) in
				...
				begin
					...
					let string = "<var loc='"^(string_of_loc loc)^(*"' level='"^(string_of_int !level)^*)"'><name>"^(escape_string name)^"</name></var>"  in
						listvars := List.append !listvars ((!level, string)::[]);
...
and print_class_type f = function (* The type of class types                                    *)
	| CtNil(loc) -> ()
	(* (virtual)? i ([ t ])? *)
	| CtCon(loc, meta_bool1, ident1, ctyp1) -> print_ident f ident1
	(* [t]) -> ct *)
...
and print_class_expr f = function (* The type of class expressions                              *)
	...
	(* (virtual)? i ([ t ])? *)
	| CeCon(loc, meta_bool1, ident1, ctyp1) -> print_ident f ident1;
	(* fun p -> ce *)
...
and print_class_str_item f = function (* The type of class structure items                          *)
	...
	| CrMth(loc, name, meta_bool1, expr1, ctyp1) -> print_expr f expr1
	(* value (mutable)? s = e *)
...
let print_astvarocc_in_xml channel argument argument2=
	level := 0;
	varname := "";
	varloc := "";
	varlevel := 0;
