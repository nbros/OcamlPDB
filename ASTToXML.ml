...
and print_ctyp f = function (* Representation of types                                     *)
    (** Empty type *)
    | TyNil(loc) -> pp f "<TyNil loc='%s'></TyNil>" (string_of_loc loc) 
    ...
    (* _ *) (** Wildcard *)
    | TyAny(loc) -> pp f "<TyAny loc='%s'></TyAny>" (string_of_loc loc) 
    ...
    (* #i *) (* #point *) (** Class type *)
    | TyCls(loc, ident1) -> pp f "<TyCls loc='%s'><ident1>%a</ident1></TyCls>" (string_of_loc loc) print_ident ident1 
    (* ~s:t *) (** Label type *)
    | TyLab(loc, name, ctyp1) -> pp f "<TyLab loc='%s'><name>%s</name><ctyp1>%a</ctyp1></TyLab>" (string_of_loc loc) (escape_string name) print_ctyp ctyp1 
    (* i *) (* Lazy.t *) (** Type identifier *)
		| TyId(loc, ident1) -> pp f "<TyId loc='%s'><ident1>%a</ident1></TyId>" (string_of_loc loc) print_ident ident1 
    ...
    (* type t 'a 'b 'c = t constraint t = t constraint t = t *) (** Type declaration *)
		| TyDcl(loc, name, (*TODO*) ctyps, ctyp1, (*TODO*) constraints) -> pp f "<TyDcl loc='%s'><name>%s</name><ctyps>%a</ctyps><ctyp1>%a</ctyp1><constraints>%a</constraints></TyDcl>" (string_of_loc loc) (escape_string name) print_ctyps ctyps print_ctyp ctyp1 print_constraints constraints 
    (* < (t)? (..)? > *) (* < move : int) -> 'a .. > as 'a  *) (**   Object type *)
    | TyObj(loc, ctyp1, meta_bool1) -> pp f "<TyObj loc='%s'><ctyp1>%a</ctyp1><meta_bool1>%a</meta_bool1></TyObj>" (string_of_loc loc) print_ctyp ctyp1 print_meta_bool meta_bool1 
    (* ?s:t *) (** Optional label type *)
    | TyOlb(loc, name, ctyp1) -> pp f "<TyOlb loc='%s'><name>%s</name><ctyp1>%a</ctyp1></TyOlb>" (string_of_loc loc) (escape_string name) print_ctyp ctyp1 
    ...
    | TyRec(loc, ctyp1) -> pp f "<TyRec loc='%s'><ctyp1>%a</ctyp1></TyRec>" (string_of_loc loc) print_ctyp ctyp1 
    ...
    | TySum(loc, ctyp1) -> pp f "<TySum loc='%s'><ctyp1>%a</ctyp1></TySum>" (string_of_loc loc) print_ctyp ctyp1 
    (* t of t *) (* A of int *)
    ...
    | TyPrv(loc, ctyp1) -> pp f "<TyPrv loc='%s'><ctyp1>%a</ctyp1></TyPrv>" (string_of_loc loc) print_ctyp ctyp1 
    (* mutable t *) (** Mutable type *)
    | TyMut(loc, ctyp1) -> pp f "<TyMut loc='%s'><ctyp1>%a</ctyp1></TyMut>" (string_of_loc loc) print_ctyp ctyp1 
    (* ( t ) *) (* (int * string) *) (** Tuple *)
    | TyTup(loc, ctyp1) -> pp f "<TyTup loc='%s'><ctyp1>%a</ctyp1></TyTup>" (string_of_loc loc) print_ctyp ctyp1 
    (* t * t *)
    ...
    (* [ = t ] *)
    | TyVrnEq(loc, ctyp1) -> pp f "<TyVrnEq loc='%s'><ctyp1>%a</ctyp1></TyVrnEq>" (string_of_loc loc) print_ctyp ctyp1 
    (* [ > t ] *)
    | TyVrnSup(loc, ctyp1) -> pp f "<TyVrnSup loc='%s'><ctyp1>%a</ctyp1></TyVrnSup>" (string_of_loc loc) print_ctyp ctyp1 
    (* [ < t ] *)
    | TyVrnInf(loc, ctyp1) -> pp f "<TyVrnInf loc='%s'><ctyp1>%a</ctyp1></TyVrnInf>" (string_of_loc loc) print_ctyp ctyp1 
    (* [ < t > t ] *)
   ...
   and print_patt f = function (* The type of patterns                                       *)
    (**   Empty pattern *)
    | PaNil(loc) -> pp f "<PaNil loc='%s'></PaNil>" (string_of_loc loc) 
    (* i *) (** Identifier *)
		| PaId(loc, ident1) -> pp f "<PaId loc='%s'><ident1>%a</ident1></PaId>" (string_of_loc loc) print_ident ident1 
    ...
    (* [| p |] *) (** Array *)
    | PaArr(loc, patt1) -> pp f "<PaArr loc='%s'><patt1>%a</patt1></PaArr>" (string_of_loc loc) print_patt patt1 
    ...
    (* ~s or ~s:(p) *) (** Label *)
    | PaLab(loc, name, patt1) -> pp f "<PaLab loc='%s'><name>%s</name><patt1>%a</patt1></PaLab>" (string_of_loc loc) (escape_string name) print_patt patt1 
    (* ?s or ?s:(p) *) (** Optional label *)
    | PaOlb(loc, name, patt1) -> pp f "<PaOlb loc='%s'><name>%s</name><patt1>%a</patt1></PaOlb>" (string_of_loc loc) (escape_string name) print_patt patt1 
    (* ?s:(p = e) or ?(p = e) *) (** Optional label with default value *)
    | PaOlbi(loc, name, patt1, expr1) -> pp f "<PaOlbi loc='%s'><name>%s</name><patt1>%a</patt1><expr1>%a</expr1></PaOlbi>" (string_of_loc loc) (escape_string name) print_patt patt1 print_expr expr1 
    (* p | p *) (** Or *)
    ...
    (* { p } *) (** Record *)
    | PaRec(loc, patt1) -> pp f "<PaRec loc='%s'><patt1>%a</patt1></PaRec>" (string_of_loc loc) print_patt patt1 
    (* i = p *) (** Equality *)
		| PaEq(loc, ident1, patt1) -> pp f "<PaEq loc='%s'><ident1>%a</ident1><patt1>%a</patt1></PaEq>" (string_of_loc loc) print_ident ident1 print_patt patt1 
    (* s *) (** String *)
    | PaStr(loc, name) -> pp f "<PaStr loc='%s'><name>%s</name></PaStr>" (string_of_loc loc) (escape_string name) 
    (* ( p ) *) (** Tuple *)
    | PaTup(loc, patt1) -> pp f "<PaTup loc='%s'><patt1>%a</patt1></PaTup>" (string_of_loc loc) print_patt patt1 
    (* (p : t) *) (** Type constraint *)
    | PaTyc(loc, patt1, ctyp1) -> pp f "<PaTyc loc='%s'><patt1>%a</patt1><ctyp1>%a</ctyp1></PaTyc>" (string_of_loc loc) print_patt patt1 print_ctyp ctyp1 
    (* #i *)
    | PaTyp(loc, ident1) -> pp f "<PaTyp loc='%s'><ident1>%a</ident1></PaTyp>" (string_of_loc loc) print_ident ident1 
    (* `s *) (** Polymorphic variant *)
    | PaVrn(loc, name) -> pp f "<PaVrn loc='%s'><name>%s</name></PaVrn>" (string_of_loc loc) (escape_string name) 
    (* lazy p *)
    | PaLaz(loc, patt1) -> pp f "<PaLaz loc='%s'><patt1>%a</patt1></PaLaz>" (string_of_loc loc) print_patt patt1 
  and print_expr f = function (* The type of expressions                                    *)
    (** Empty expression *)
    | ExNil(loc) -> pp f "<ExNil loc='%s'></ExNil>" (string_of_loc loc) 
    (* i *) (**   Identifier *)
		| ExId(loc, ident1) -> pp f "<ExId loc='%s'><ident1>%a</ident1></ExId>" (string_of_loc loc) print_ident ident1 
    ...
    (* [| e |] *) (** Array declaration *)
    | ExArr(loc, expr1) -> pp f "<ExArr loc='%s'><expr1>%a</expr1></ExArr>" (string_of_loc loc) print_expr expr1 
    ...
    (* assert e *) (** assert e *)
    | ExAsr(loc, expr1) -> pp f "<ExAsr loc='%s'><expr1>%a</expr1></ExAsr>" (string_of_loc loc) print_expr expr1 
    (* e := e *) (** Assignment *)
    ...
    | ExCoe(loc, expr1, ctyp1, ctyp2) -> pp f "<ExCoe loc='%s'><expr1>%a</expr1><ctyp1>%a</ctyp1><ctyp2>%a</ctyp2></ExCoe>" (string_of_loc loc) print_expr expr1 print_ctyp ctyp1 print_ctyp ctyp2 
    (* 3.14 *) (** Float *)
    | ExFlo(loc, name) -> pp f "<ExFlo loc='%s'><name>%s</name></ExFlo>" (string_of_loc loc) (escape_string name) 
    (* for s = e to/downto e do { e } *) (** For loop *)
    | ExFor(loc, name, expr1, expr2, meta_bool1, expr3) -> pp f "<ExFor loc='%s'><name>%s</name><expr1>%a</expr1><expr2>%a</expr2><meta_bool1>%a</meta_bool1><expr3>%a</expr3></ExFor>" (string_of_loc loc) (escape_string name) print_expr expr1 print_expr expr2 print_meta_bool meta_bool1 print_expr expr3 
    (* fun [ mc ] *) (** Function with match case *)
    | ExFun(loc, match_case1) -> pp f "<ExFun loc='%s'><match_case1>%a</match_case1></ExFun>" (string_of_loc loc) print_match_case match_case1 
    ...
    | ExLab(loc, name, expr1) -> pp f "<ExLab loc='%s'><name>%s</name><expr1>%a</expr1></ExLab>" (string_of_loc loc) (escape_string name) print_expr expr1 
    (* lazy e *) (** Lazy evaluation *)
    | ExLaz(loc, expr1) -> pp f "<ExLaz loc='%s'><expr1>%a</expr1></ExLaz>" (string_of_loc loc) print_expr expr1 
    (* let b in e or let rec b in e *) (** Let statement with/without recursion *)
    | ExLet(loc, meta_bool1, binding1, expr1) -> pp f "<ExLet loc='%s'><meta_bool1>%a</meta_bool1><binding1>%a</binding1><expr1>%a</expr1></ExLet>" (string_of_loc loc) print_meta_bool meta_bool1 print_binding binding1 print_expr expr1 
    (* let module s = me in e *) (** "Let module in" construct *)
    | ExLmd(loc, name, module_expr1, expr1) -> pp f "<ExLmd loc='%s'><name>%s</name><module_expr1>%a</module_expr1><expr1>%a</expr1></ExLmd>" (string_of_loc loc) (escape_string name) print_module_expr module_expr1 print_expr expr1 
    (* match e with [ mc ] *) (** Match case *)
    | ExMat(loc, expr1, match_case1) -> pp f "<ExMat loc='%s'><expr1>%a</expr1><match_case1>%a</match_case1></ExMat>" (string_of_loc loc) print_expr expr1 print_match_case match_case1 
    (* new i *) (** New object *)
    | ExNew(loc, ident1) -> pp f "<ExNew loc='%s'><ident1>%a</ident1></ExNew>" (string_of_loc loc) print_ident ident1 
    (* object ((p))? (cst)? end *) (** Object declaration *)
    | ExObj(loc, patt1, class_str_item1) -> pp f "<ExObj loc='%s'><patt1>%a</patt1><class_str_item1>%a</class_str_item1></ExObj>" (string_of_loc loc) print_patt patt1 print_class_str_item class_str_item1 
    (* ?s or ?s:e *) (** Optional label *)
    | ExOlb(loc, name, expr1) -> pp f "<ExOlb loc='%s'><name>%s</name><expr1>%a</expr1></ExOlb>" (string_of_loc loc) (escape_string name) print_expr expr1 
    (* {< rb >} *) (** Overloading *)
    | ExOvr(loc, rec_binding1) -> pp f "<ExOvr loc='%s'><rec_binding1>%a</rec_binding1></ExOvr>" (string_of_loc loc) print_rec_binding rec_binding1 
    (* { rb } or { (e) with rb } *) (** Record *)
    | ExRec(loc, rec_binding1, expr1) -> pp f "<ExRec loc='%s'><rec_binding1>%a</rec_binding1><expr1>%a</expr1></ExRec>" (string_of_loc loc) print_rec_binding rec_binding1 print_expr expr1 
    (* do { e } *) (** Sequence with "do" statement *)
    | ExSeq(loc, expr1) -> pp f "<ExSeq loc='%s'><expr1>%a</expr1></ExSeq>" (string_of_loc loc) print_expr expr1 
    (* e#s *) (** Method call *)
    | ExSnd(loc, expr1, name) -> pp f "<ExSnd loc='%s'><expr1>%a</expr1><name>%s</name></ExSnd>" (string_of_loc loc) print_expr expr1 (escape_string name) 
    (* e.[e] *) (** String access *)
    ...
    | ExTry(loc, expr1, match_case1) -> pp f "<ExTry loc='%s'><expr1>%a</expr1><match_case1>%a</match_case1></ExTry>" (string_of_loc loc) print_expr expr1 print_match_case match_case1 
    (* (e) *) (** Tuple *)
    | ExTup(loc, expr1) -> pp f "<ExTup loc='%s'><expr1>%a</expr1></ExTup>" (string_of_loc loc) print_expr expr1 
    ...
    (* (e : t) *) (** Type constraint *)
    | ExTyc(loc, expr1, ctyp1) -> pp f "<ExTyc loc='%s'><expr1>%a</expr1><ctyp1>%a</ctyp1></ExTyc>" (string_of_loc loc) print_expr expr1 print_ctyp ctyp1 
    (* `s *) (** Polymorphic variant *)
  ...
  and print_module_type f = function (* The type of module types                                   *)
    ...
    (* i *) (* A.B.C *)
		| MtId(loc, ident1) -> pp f "<MtId loc='%s'><ident1>%a</ident1></MtId>" (string_of_loc loc) print_ident ident1 
    (* functor (s : mt)) -> mt *)
    ...
    (* sig sg end *)
    | MtSig(loc, sig_item1) -> pp f "<MtSig loc='%s'><sig_item1>%a</sig_item1></MtSig>" (string_of_loc loc) print_sig_item sig_item1 
    (* mt with wc *)
    | MtWit(loc, module_type1, with_constr1) -> pp f "<MtWit loc='%s'><module_type1>%a</module_type1><with_constr1>%a</with_constr1></MtWit>" (string_of_loc loc) print_module_type module_type1 print_with_constr with_constr1 
    (* $s$ *)
  ...
  and print_sig_item f = function (* The type of signature items                                *)
    ...
    (* class cict *)
    | SgCls(loc, class_type1) -> pp f "<SgCls loc='%s'><class_type1>%a</class_type1></SgCls>" (string_of_loc loc) print_class_type class_type1 
    (* class type cict *)
    | SgClt(loc, class_type1) -> pp f "<SgClt loc='%s'><class_type1>%a</class_type1></SgClt>" (string_of_loc loc) print_class_type class_type1 
    (* sg ; sg *)
    ...
    (* # s or # s e *)
    | SgDir(loc, name, expr1) -> pp f "<SgDir loc='%s'><name>%s</name><expr1>%a</expr1></SgDir>" (string_of_loc loc) (escape_string name) print_expr expr1 
    (* exception t *)
    | SgExc(loc, ctyp1) -> pp f "<SgExc loc='%s'><ctyp1>%a</ctyp1></SgExc>" (string_of_loc loc) print_ctyp ctyp1 
    (* external s : t = s ... s *)
    | SgExt(loc, name, ctyp1, strings (*meta_list string*)) -> pp f "<SgExt loc='%s'><name>%s</name><ctyp1>%a</ctyp1><strings>%a</strings></SgExt>" (string_of_loc loc) (escape_string name) print_ctyp ctyp1 print_strings strings
    (* include mt *)
    | SgInc(loc, module_type1) -> pp f "<SgInc loc='%s'><module_type1>%a</module_type1></SgInc>" (string_of_loc loc) print_module_type module_type1 
    (* module s : mt *)
    | SgMod(loc, name, module_type1) -> pp f "<SgMod loc='%s'><name>%s</name><module_type1>%a</module_type1></SgMod>" (string_of_loc loc) (escape_string name) print_module_type module_type1 
    (* module rec mb *)
    | SgRecMod(loc, module_binding1) -> pp f "<SgRecMod loc='%s'><module_binding1>%a</module_binding1></SgRecMod>" (string_of_loc loc) print_module_binding module_binding1 
    (* module type s = mt *)
    | SgMty(loc, name, module_type1) -> pp f "<SgMty loc='%s'><name>%s</name><module_type1>%a</module_type1></SgMty>" (string_of_loc loc) (escape_string name) print_module_type module_type1 
    (* open i *)
    | SgOpn(loc, ident1) -> pp f "<SgOpn loc='%s'><ident1>%a</ident1></SgOpn>" (string_of_loc loc) print_ident ident1 
    (* type t *)
    | SgTyp(loc, ctyp1) -> pp f "<SgTyp loc='%s'><ctyp1>%a</ctyp1></SgTyp>" (string_of_loc loc) print_ctyp ctyp1 
    (* value s : t *)
    | SgVal(loc, name, ctyp1) -> pp f "<SgVal loc='%s'><name>%s</name><ctyp1>%a</ctyp1></SgVal>" (string_of_loc loc) (escape_string name) print_ctyp ctyp1 
    (* $s$ *)
  ...
  and print_with_constr f = function (* The type of `with' constraints                             *)
    | WcNil(loc) -> pp f "<WcNil loc='%s'></WcNil>" (string_of_loc loc) 
    (* type t = t *)
  ...
  and print_binding f = function (* The type of let bindings                                   *)
    | BiNil(loc) -> pp f "<BiNil loc='%s'></BiNil>" (string_of_loc loc) 
    ...
    (* p = e *) (* let patt = expr *)
		| BiEq(loc, patt1, expr1) -> pp f "<BiEq loc='%s'><patt1>%a</patt1><expr1>%a</expr1></BiEq>" (string_of_loc loc) print_patt patt1 print_expr expr1 
    (* $s$ *)
  ...
  and print_rec_binding f = function (* The type of record definitions                             *)
    (** Empty record definition *)
		| RbNil(loc) -> pp f "<RbNil loc='%s'></RbNil>" (string_of_loc loc) 
    (* rb ; rb *)
    ...
    (* i = e *)
		| RbEq(loc, ident1, expr1) -> pp f "<RbEq loc='%s'><ident1>%a</ident1><expr1>%a</expr1></RbEq>" (string_of_loc loc) print_ident ident1 print_expr expr1 
    (* $s$ *)
  ...
  and print_module_binding f = function (* The type of recursive module definitions                   *)
    (** Empty module definition *)
		| MbNil(loc) -> pp f "<MbNil loc='%s'></MbNil>" (string_of_loc loc) 
    ...
    (* s : mt = me *)
		| MbColEq(loc, name, module_type1, module_expr1) -> pp f "<MbColEq loc='%s'><name>%s</name><module_type1>%a</module_type1><module_expr1>%a</module_expr1></MbColEq>" (string_of_loc loc) (escape_string name) print_module_type module_type1 print_module_expr module_expr1 
    (* s : mt *)
		| MbCol(loc, name, module_type1) -> pp f "<MbCol loc='%s'><name>%s</name><module_type1>%a</module_type1></MbCol>" (string_of_loc loc) (escape_string name) print_module_type module_type1 
    (* $s$ *)
  ...
  and print_match_case f = function (* The type of cases for match/function/try constructions     *)
		(** Empty case *)
    | McNil(loc) -> pp f "<McNil loc='%s'></McNil>" (string_of_loc loc) 
    (* a | a *)
    ...
    (* p (when e)?) -> e *)
    | McArr(loc, patt1, expr1, expr2) -> pp f "<McArr loc='%s'><patt1>%a</patt1><expr1>%a</expr1><expr2>%a</expr2></McArr>" (string_of_loc loc) print_patt patt1 print_expr expr1 print_expr expr2 
    (* $s$ *)
  ...
  and print_module_expr f = function (* The type of module expressions                             *)
    (** Empty module expression *)
		| MeNil(loc) -> pp f "<MeNil loc='%s'></MeNil>" (string_of_loc loc) 
    (* i *)
		| MeId(loc, ident1) -> pp f "<MeId loc='%s'><ident1>%a</ident1></MeId>" (string_of_loc loc) print_ident ident1 
    (* me me *)
    ...
    (* functor (s : mt)) -> me *)
    | MeFun(loc, name, module_type1, module_expr1) -> pp f "<MeFun loc='%s'><name>%s</name><module_type1>%a</module_type1><module_expr1>%a</module_expr1></MeFun>" (string_of_loc loc) (escape_string name) print_module_type module_type1 print_module_expr module_expr1 
    (* struct st end *)
    | MeStr(loc, str_item1) -> pp f "<MeStr loc='%s'><str_item1>%a</str_item1></MeStr>" (string_of_loc loc) print_str_item str_item1 
    (* (me : mt) *)
    ...
    | StNil(loc) -> pp f "<StNil loc='%s'></StNil>" (string_of_loc loc) 
    (* class cice *)
    | StCls(loc, class_expr1) -> pp f "<StCls loc='%s'><class_expr1>%a</class_expr1></StCls>" (string_of_loc loc) print_class_expr class_expr1 
    (* class type cict *)
    | StClt(loc, class_type1) -> pp f "<StClt loc='%s'><class_type1>%a</class_type1></StClt>" (string_of_loc loc) print_class_type class_type1 
    (* st ; st *)
    ...
    (* # s or # s e *)
    | StDir(loc, name, expr1) -> pp f "<StDir loc='%s'><name>%s</name><expr1>%a</expr1></StDir>" (string_of_loc loc) (escape_string name) print_expr expr1 
    (* exception t or exception t = i *)
    | StExc(loc, ctyp1, option_ident (*meta_option ident*)) -> pp f "<StExc loc='%s'><ctyp1>%a</ctyp1><option_ident>%a</option_ident></StExc>" (string_of_loc loc) print_ctyp ctyp1 print_option_ident option_ident
    (* e *)
    | StExp(loc, expr1) -> pp f "<StExp loc='%s'><expr1>%a</expr1></StExp>" (string_of_loc loc) print_expr expr1 
    (* external s : t = s ... s *)
    | StExt(loc, name, ctyp1, (*TODO*) strings(*meta_list string*)) -> pp f "<StExt loc='%s'><name>%s</name><ctyp1>%a</ctyp1><strings>%a</strings></StExt>" (string_of_loc loc) (escape_string name) print_ctyp ctyp1 print_strings strings 
    (* include me *)
    | StInc(loc, module_expr1) -> pp f "<StInc loc='%s'><module_expr1>%a</module_expr1></StInc>" (string_of_loc loc) print_module_expr module_expr1 
    (* module s = me *)
    ...
    (* module rec mb *)
    | StRecMod(loc, module_binding1) -> pp f "<StRecMod loc='%s'><module_binding1>%a</module_binding1></StRecMod>" (string_of_loc loc) print_module_binding module_binding1 
    (* module type s = mt *)
    | StMty(loc, name, module_type1) -> pp f "<StMty loc='%s'><name>%s</name><module_type1>%a</module_type1></StMty>" (string_of_loc loc) (escape_string name) print_module_type module_type1 
    (* open i *)
    | StOpn(loc, ident1) -> pp f "<StOpn loc='%s'><ident1>%a</ident1></StOpn>" (string_of_loc loc) print_ident ident1 
    (* type t *)
    | StTyp(loc, ctyp1) -> pp f "<StTyp loc='%s'><ctyp1>%a</ctyp1></StTyp>" (string_of_loc loc) print_ctyp ctyp1 
    (* value (rec)? bi *)
    | StVal(loc, meta_bool1, binding1) -> pp f "<StVal loc='%s'><meta_bool1>%a</meta_bool1><binding1>%a</binding1></StVal>" (string_of_loc loc) print_meta_bool meta_bool1 print_binding binding1 
    (* $s$ *)
  ...
  and print_class_type f = function (* The type of class types                                    *)
    | CtNil(loc) -> pp f "<CtNil loc='%s'></CtNil>" (string_of_loc loc) 
    (* (virtual)? i ([ t ])? *)
    | CtCon(loc, meta_bool1, ident1, ctyp1) -> pp f "<CtCon loc='%s'><meta_bool1>%a</meta_bool1><ident1>%a</ident1><ctyp1>%a</ctyp1></CtCon>" (string_of_loc loc) print_meta_bool meta_bool1 print_ident ident1 print_ctyp ctyp1 
    (* [t]) -> ct *)
    | CtFun(loc, ctyp1, class_type1) -> pp f "<CtFun loc='%s'><ctyp1>%a</ctyp1><class_type1>%a</class_type1></CtFun>" (string_of_loc loc) print_ctyp ctyp1 print_class_type class_type1 
    (* object ((t))? (csg)? end *)
    | CtSig(loc, ctyp1, class_sig_item1) -> pp f "<CtSig loc='%s'><ctyp1>%a</ctyp1><class_sig_item1>%a</class_sig_item1></CtSig>" (string_of_loc loc) print_ctyp ctyp1 print_class_sig_item class_sig_item1 
    (* ct, ct *)
  ...
  and print_class_sig_item f = function (* The type of class signature items                          *)
    | CgNil(loc) -> pp f "<CgNil loc='%s'></CgNil>" (string_of_loc loc) 
    (* type t = t *)
    ...
    (* inherit ct *)
    | CgInh(loc, class_type1) -> pp f "<CgInh loc='%s'><class_type1>%a</class_type1></CgInh>" (string_of_loc loc) print_class_type class_type1 
    (* method s : t or method private s : t *)
    | CgMth(loc, name, meta_bool1, ctyp1) -> pp f "<CgMth loc='%s'><name>%s</name><meta_bool1>%a</meta_bool1><ctyp1>%a</ctyp1></CgMth>" (string_of_loc loc) (escape_string name) print_meta_bool meta_bool1 print_ctyp ctyp1 
    (* value (virtual)? (mutable)? s : t *)
    | CgVal(loc, name, meta_bool1, meta_bool2, ctyp1) -> pp f "<CgVal loc='%s'><name>%s</name><meta_bool1>%a</meta_bool1><meta_bool2>%a</meta_bool2><ctyp1>%a</ctyp1></CgVal>" (string_of_loc loc) (escape_string name) print_meta_bool meta_bool1 print_meta_bool meta_bool2 print_ctyp ctyp1 
    (* method virtual (mutable)? s : t *)
    | CgVir(loc, name, meta_bool1, ctyp1) -> pp f "<CgVir loc='%s'><name>%s</name><meta_bool1>%a</meta_bool1><ctyp1>%a</ctyp1></CgVir>" (string_of_loc loc) (escape_string name) print_meta_bool meta_bool1 print_ctyp ctyp1 
    (* $s$ *)
  ...
  and print_class_expr f = function (* The type of class expressions                              *)
    | CeNil(loc) -> pp f "<CeNil loc='%s'></CeNil>" (string_of_loc loc) 
    (* ce e *)
    | CeApp(loc, class_expr1, expr1) -> pp f "<CeApp loc='%s'><class_expr1>%a</class_expr1><expr1>%a</expr1></CeApp>" (string_of_loc loc) print_class_expr class_expr1 print_expr expr1 
    (* (virtual)? i ([ t ])? *)
    | CeCon(loc, meta_bool1, ident1, ctyp1) -> pp f "<CeCon loc='%s'><meta_bool1>%a</meta_bool1><ident1>%a</ident1><ctyp1>%a</ctyp1></CeCon>" (string_of_loc loc) print_meta_bool meta_bool1 print_ident ident1 print_ctyp ctyp1 
    (* fun p) -> ce *)
    | CeFun(loc, patt1, class_expr1) -> pp f "<CeFun loc='%s'><patt1>%a</patt1><class_expr1>%a</class_expr1></CeFun>" (string_of_loc loc) print_patt patt1 print_class_expr class_expr1 
    (* let (rec)? bi in ce *)
    | CeLet(loc, meta_bool1, binding1, class_expr1) -> pp f "<CeLet loc='%s'><meta_bool1>%a</meta_bool1><binding1>%a</binding1><class_expr1>%a</class_expr1></CeLet>" (string_of_loc loc) print_meta_bool meta_bool1 print_binding binding1 print_class_expr class_expr1 
    (* object ((p))? (cst)? end *)
    | CeStr(loc, patt1, class_str_item1) -> pp f "<CeStr loc='%s'><patt1>%a</patt1><class_str_item1>%a</class_str_item1></CeStr>" (string_of_loc loc) print_patt patt1 print_class_str_item class_str_item1 
    (* ce : ct *)
    | CeTyc(loc, class_expr1, class_type1) -> pp f "<CeTyc loc='%s'><class_expr1>%a</class_expr1><class_type1>%a</class_type1></CeTyc>" (string_of_loc loc) print_class_expr class_expr1 print_class_type class_type1 
    (* ce, ce *)
  ...
  and print_class_str_item f = function (* The type of class structure items                          *)
    | CrNil(loc) -> pp f "<CrNil loc='%s'></CrNil>" (string_of_loc loc) 
    (* cst ; cst *)
    ...
    | CrInh(loc, class_expr1, name) -> pp f "<CrInh loc='%s'><class_expr1>%a</class_expr1><name>%s</name></CrInh>" (string_of_loc loc) print_class_expr class_expr1 (escape_string name) 
    (* initializer e *)
    | CrIni(loc, expr1) -> pp f "<CrIni loc='%s'><expr1>%a</expr1></CrIni>" (string_of_loc loc) print_expr expr1 
    (* method (private)? s : t = e or method (private)? s = e *)
    | CrMth(loc, name, meta_bool1, expr1, ctyp1) -> pp f "<CrMth loc='%s'><name>%s</name><meta_bool1>%a</meta_bool1><expr1>%a</expr1><ctyp1>%a</ctyp1></CrMth>" (string_of_loc loc) (escape_string name) print_meta_bool meta_bool1 print_expr expr1 print_ctyp ctyp1 
    (* value (mutable)? s = e *)
    | CrVal(loc, name, meta_bool1, expr1) -> pp f "<CrVal loc='%s'><name>%s</name><meta_bool1>%a</meta_bool1><expr1>%a</expr1></CrVal>" (string_of_loc loc) (escape_string name) print_meta_bool meta_bool1 print_expr expr1 
    (* method virtual (private)? s : t *)
    | CrVir(loc, name, meta_bool1, ctyp1) -> pp f "<CrVir loc='%s'><name>%s</name><meta_bool1>%a</meta_bool1><ctyp1>%a</ctyp1></CrVir>" (string_of_loc loc) (escape_string name) print_meta_bool meta_bool1 print_ctyp ctyp1 
    (* value virtual (private)? s : t *)
    | CrVvr(loc, name, meta_bool1, ctyp1) -> pp f "<CrVvr loc='%s'><name>%s</name><meta_bool1>%a</meta_bool1><ctyp1>%a</ctyp1></CrVvr>" (string_of_loc loc) (escape_string name) print_meta_bool meta_bool1 print_ctyp ctyp1 
    (* $s$ *)
